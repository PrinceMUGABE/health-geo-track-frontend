...
const processGraphData = () => {
  ...
  // Cases by Facility Bar Chart
  const facilityData = incidents.reduce((acc, incident) => {
    const facilityName = incident.health_facility?.name || 'Unknown';
    acc[facilityName] = (acc[facilityName] || 0) + incident.number_of_cases;
    return acc;
  }, {});

  const facilityBarData = Object.entries(facilityData)
    .map(([name, cases]) => ({ name, cases }))
    .sort((a, b) => b.cases - a.cases)
    .slice(0, 5); // Top 5 facilities

  return { statusDistribution, facilityBarData };
};
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
          ...
          <div className="bg-white shadow rounded-lg p-4">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={processGraphData().statusDistribution}
                  cx="50%"
                  cy="50%"
                  labelLine={false}
                  outerRadius="80%"
                ...
                >
                  {processGraphData().statusDistribution.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
          ...
          {/* Top 5 Facilities by Number of Cases Bar Chart */}
          <div className="bg-white shadow rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-4 text-center">
