...
const AdminManageFacilities = () => {
  ...
  // Prepare data for Facility Type Pie Chart
  const facilityTypeData = facilities.reduce((acc, facility) => {
    const existingType = acc.find(item => item.name === facility.facility_type);
    if (existingType) {
      existingType.value += 1;
    } else {
      acc.push({ name: facility.facility_type, value: 1 });
    }
    return acc;
  }, []);

  // Prepare data for Facility Status Bar Chart
  const facilityStatusData = facilities.reduce((acc, facility) => {
    const existingStatus = acc.find(item => item.name === facility.status);
    if (existingStatus) {
      existingStatus.count += 1;
    } else {
      acc.push({ name: facility.status, count: 1 });
    }
    return acc;
  }, []);
  ...
  return (
    <div className="justify-center w-full px-12 ml-4">
      ...
      <div className="bg-white shadow rounded-lg p-4">
        <h3 className="text-lg font-semibold mb-4 text-center">Facility Type Distribution</h3>
      ...
      {/* Facility Status Bar Chart */}
      <div className="bg-white shadow rounded-lg p-4">
        <h3 className="text-lg font-semibold mb-4 text-center">Facility Status Breakdown</h3>
        <ResponsiveContainer width="100%" height={300}>
      ...
      <Dialog open={showUpdateModal} onOpenChange={setShowUpdateModal}>
        <DialogContent className="">
          <DialogHeader>
            <DialogTitle className="text-black text-center font-bold">Update Facility</DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div className="flex justify-end space-x-2">
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-black rounded-md hover:bg-green-400"
              >
                Cancel
              </button>
              <button
