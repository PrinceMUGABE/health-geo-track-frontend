...
const AdminManageHealthAccessibilities = () => {
  ...
  // New function to process graph data
  const processGraphData = () => {
    // Accessibility Rating Distribution Pie Chart
    const ratingDistribution = Object.entries(
      accessibilities.reduce((acc, a) => {
        acc[a.accessibility_rating] = (acc[a.accessibility_rating] || 0) + 1;
        return acc;
      }, {})
    ).map(([rating, count]) => ({
      name: rating,
    ...
    // Average Travel Time by Facility Bar Chart
    const facilityTravelData = accessibilities.reduce((acc, a) => {
      const facilityName = a.health_facility?.name || 'Unknown';
      acc[facilityName] = (acc[facilityName] || 0) + parseFloat(a.avg_travel_time);
      return acc;
    }, {});

    const facilityBarData = Object.entries(facilityTravelData)
      .map(([name, avgTime]) => ({ 
        name, 
        avgTravelTime: (avgTime / accessibilities.filter(a => a.health_facility?.name === name).length).toFixed(1)
      }))
      .sort((a, b) => b.avgTravelTime - a.avgTravelTime)
      .slice(0, 5); // Top 5 facilities

    return { ratingDistribution, facilityBarData };
  };

  // Colors for pie chart
  const COLORS = ['#0E9F6E', '#1C64F2', '#5145CD'];
    ...
    <div className="w-full px-12 ml-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-4 space-y-2 md:space-y-0">
        <button
          ...
          className="px-4 py-2 bg-blue-400 text-white rounded-md flex items-center space-x-2 hover:bg-black"
        >
      ...
      <div className="flex justify-between items-center mt-4">
       ...
       {/* Graphs Section */}
       {!loading && accessibilities.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
          {/* Accessibility Rating Distribution Pie Chart */}
          <div className="bg-white shadow rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-4 text-center">
              Accessibility Rating Distribution
            </h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={processGraphData().ratingDistribution}
                  cx="50%"
                  cy="50%"
                  labelLine={false}
                  outerRadius="80%"
                ...
                >
                  {processGraphData().ratingDistribution.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
          ...
          <div className="bg-white shadow rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-4 text-center">
              Average Travel Time by Facility
            </h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart 
                data={processGraphData().facilityBarData}
              >
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="name" />
                <YAxis label={{ value: 'Avg Travel Time (mins)', angle: -90, position: 'insideLeft' }} />
                <Tooltip />
                <Legend />
                <Bar dataKey="avgTravelTime" fill="#8884d8" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
...
export default AdminManageHealthAccessibilities;