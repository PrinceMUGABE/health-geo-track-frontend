...
const AdminManagePopulations = () => {
  ...
  // New function to prepare data for population distribution charts
  const preparePopulationChartData = () => {
    // Socioeconomic Status Distribution
    const socioeconomicDistribution = populations.reduce((acc, pop) => {
      const status = pop.socioeconomic_status;
      acc[status] = (acc[status] || 0) + pop.total_population;
      return acc;
    }, {});

    const socioeconomicData = Object.entries(socioeconomicDistribution).map(
      ([name, value]) => ({ name, value })
    );

    // Age Group Distribution (aggregate across all populations)
    const ageGroupData = populations.reduce(
      (acc, pop) => {
        acc[0].value += parseInt(pop.children_under_5);
        acc[1].value += parseInt(pop.youth_population);
        acc[2].value += parseInt(pop.adult_population);
        acc[3].value += parseInt(pop.elderly_population);
        return acc;
      },
      [
        { name: "Children (0-5)", value: 0 },
        { name: "Youth", value: 0 },
        { name: "Adults", value: 0 },
    ...
    return { socioeconomicData, ageGroupData };
  };
    ...
    <div className="w-full px-12 ml-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-4 space-y-2 md:space-y-0">
        <button
          ...
          className="px-4 py-2 bg-blue-400 text-white rounded-md flex items-center space-x-2 hover:bg-blue-700"
        >
      ...
      {/* New Visualization Section with Responsive Layout */}
      {!loading && populations.length > 0 && (
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="border rounded-lg p-4">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={preparePopulationChartData().socioeconomicData}
                  cx="50%"
                  cy="50%"
                  labelLine={false}
                  outerRadius={80}
                  ...
                  dataKey="value"
                >
                  <Cell key="LOW" fill="#0E9F6E" />
                  <Cell key="MIDDLE" fill="#36A2EB" />
                  <Cell key="HIGH" fill="#5145CD" />
                </Pie>
                <Tooltip />
                <Legend />
              </PieChart>
          ...
          <div className="border rounded-lg p-4">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={preparePopulationChartData().ageGroupData}>
      ...
      {/* Details Modal Charts - Also made responsive */}
      {selectedPopulation && (
...
export default AdminManagePopulations;
