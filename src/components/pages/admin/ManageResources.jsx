...
const AdminManageResources = () => {
  ...
  const durationDistribution = [
    { name: '0-15 Days', value: allocations.filter(a => a.duration_in_days <= 15).length },
    { name: '16-30 Days', value: allocations.filter(a => a.duration_in_days > 15 && a.duration_in_days <= 30).length },
    { name: '31-60 Days', value: allocations.filter(a => a.duration_in_days > 30 && a.duration_in_days <= 60).length },
    { name: '60+ Days', value: allocations.filter(a => a.duration_in_days > 60).length }
  ];
  ...
  return (
    <div className="justify-center w-full px-12 ml-4">
       <div className="w-full justify-center ml-4">
      ...
      {/* Bar Chart: Personnel Count by Facility */}
      <div className="w-full md:w-1/2 p-4 bg-white rounded-lg shadow-md">
        <h2 className="text-lg font-bold mb-4 text-center">Personnel Allocation by Facility</h2>
        <BarChart width={500} height={300} data={personnelByFacility}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="name" />
          <YAxis />
          <Tooltip />
          <Legend />
          <Bar dataKey="personnelCount" fill="#8884d8" />
        </BarChart>
      </div>

      {/* Pie Chart: Allocation Duration Distribution */}
      <div className="w-full md:w-1/2 p-4 bg-white rounded-lg shadow-md">
        <h2 className="text-lg font-bold mb-4 text-center">Resource Allocation Duration</h2>
